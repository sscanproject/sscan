//
//  HExceptedTableViewController.swift
...
class HExceptedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBOutlet weak var label_except: UILabel!
    @IBOutlet weak var tableView_excepted: UITableView!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        tableView_excepted.dataSource = self
        tableView_excepted.delegate = self
        tableView_excepted.allowsSelection = true
        
        //refreshControl
        if #available(iOS 10.0, *) {
            self.tableView_excepted.refreshControl = refreshControl
        } else {
            self.tableView_excepted.addSubview(refreshControl)
        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if(self.actxuid == ""){ //防止ACTXUID空白造成錯誤
            self.callPortalinPageExceptApi()
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        self.label_except.text = "應到人數：" + String(members.count)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "ExceptedCell", for: indexPath) as! HExceptedTableViewCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ExceptedtoDetail"{
            guard  let targetVC = segue.destination as? HMemberDetailController else {
                assertionFailure("Faild to get destination")
                return
            }
            guard let selectedIndexPath = self.tableView_excepted.indexPathForSelectedRow else {
            ...
            do {
                ...
                if(codeResult.code == 200){
                    ...
                    do {
                        ...
                        let x : Int = jsondata.mblist.count
                ...
                self.tableView_excepted.reloadData()
    ...
    func callPortalinPageExceptApi() {
        let accesstoken = userDefaults.string(forKey: "access_token")
        let acno = userDefaults.string(forKey: "acno")
